...
class LightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        #blacklist_weight_modules = (FusedLayerNorm, torch.nn.Embedding)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for chunk in range(len(idxl)):
            ...
            #x = self.blocks(x, length_mask=LM(lengths))
            x = self.blocks(x)
...
class MoleculeModule(pl.LightningDataModule):
    def __init__(self,  max_len, data_path, train_args):#, val_args):
        super().__init__()
        self.data_path = data_path
        self.train_args = train_args  # dict with keys {'batch_size', 'shuffle', 'num_workers', 'pin_memory'}
        #self.val_args = val_args  # dict with keys {'batch_size', 'shuffle', 'num_workers', 'pin_memory'}
        print(train_args)
    ...
    def setup(self, stage=None):
        ...
        if 'ZINC' in self.data_path or 'zinc' in self.data_path:
            ...
            dataset_dict_test =  load_dataset(pubchem_script, data_files=pubchem_path, cache_dir=os.path.join('/tmp',getpass.getuser(), 'pubchem'), split='train[-1%:]')
        ...
        data_path='../data/validation/dataset_v1.csv'
        self.val = load_dataset('./val_script.py', data_files=data_path, cache_dir=os.path.join('/tmp',getpass.getuser(), 'test'),split='train')
        self.pubchem= dataset_dict
        #self.pubchem_val= dataset_dict_test
        print(dataset_dict.cache_files)
        self.cache_files = []
        for cache in self.val.cache_files:
    ...
    def val_dataloader(self):
        ...
        loader =  DataLoader(self.val, collate_fn=self.text_encoder.process, **self.train_args)
        print(len(loader))
        return loader
