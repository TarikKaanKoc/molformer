...
class RecurrentTransformerEncoderLayer(Module):
    ...
    def forward(self, x, state=None, memory=None):
        ...
        """
        #this is needed for huggingface compatibility 
        if layer_past != None:
            state = layer_past
...
class RecurrentTransformerEncoderPPLM(Module):
    ...
    def __init__(self, layers, norm_layer=None, event_dispatcher=""):
        super(RecurrentTransformerEncoderPPLM, self).__init__()
    ...
    def forward(self, x, state=None, memory=None):
        ...
        #collect output (hiddens) from each layer for pplm classifier
        all_hiddens = ()
        ...
        if self.norm is not None:
            x = self.norm(x)
        #add final all_hiddens after norm similar to huggingface gpt2 code
        all_hiddens += (x,)

        return x, state, all_hiddens
